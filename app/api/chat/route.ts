...
Just run the tool and provide the answer.`,
		tools: {
			...
			nearby_search: tool({
				...
				parameters: z.object({
					location: z
						.string()
						.describe(
							"The location to search near (e.g., 'New York City' or '1600 Amphitheatre Parkway, Mountain View, CA').",
						),
					type: z
						.string()
						.describe(
							"The type of place to search for (e.g., restaurant, cafe, park).",
						),
					keyword: z
						.string()
						.optional()
						.describe("An optional keyword to refine the search."),
					radius: z
						.number()
						.default(3000)
						.describe(
							"The radius of the search area in meters (max 50000, default 3000).",
						),
				}),
				execute: async ({
					location,
			...
			find_place: tool({
				...
				parameters: z.object({
					input: z
						.string()
						.describe(
							"The place to search for (e.g., 'Museum of Contemporary Art Australia').",
						),
					inputtype: z
						.enum(["textquery", "phonenumber"])
						.describe("The type of input (textquery or phonenumber)."),
				}),
				execute: async ({
					input,
					inputtype,
				}: { input: string; inputtype: "textquery" | "phonenumber" }) => {
					const apiKey = env.GOOGLE_MAPS_API_KEY;
			...
			text_search: tool({
				description:
					"Perform a text-based search for places using Google Maps API.",
				parameters: z.object({
					query: z
						.string()
						.describe("The search query (e.g., '123 main street')."),
					location: z
						.string()
						.optional()
						.describe(
							"The location to center the search (e.g., '42.3675294,-71.186966').",
						),
					radius: z
						.number()
						.optional()
						.describe("The radius of the search area in meters (max 50000)."),
				}),
				execute: async ({
					query,
					location,
