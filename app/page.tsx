...
import { type Message, suggestQuestions } from "./actions";
...
export default function Home() {
		...
		onFinish: async (message, { finishReason }) => {
			if (finishReason === "stop") {
				const newHistory: Message[] = [
					{ role: "user", content: lastSubmittedQuery },
	...
	const WeatherChart: React.FC<{ result: any }> = React.memo(({ result }) => {
		const { chartData, minTemp, maxTemp } = useMemo(() => {
			...
			const groupedData: { [key: string]: WeatherDataPoint } =
				weatherData.reduce(
					(acc, curr) => {
						if (!acc[curr.date]) {
							acc[curr.date] = { ...curr };
	...
	const loadGoogleMapsScript = (callback: () => void) => {
		...
		script.src = `https://maps.googleapis.com/maps/api/js?key=${env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&libraries=places,marker&callback=initMap`;
		script.async = true;
		script.defer = true;
	...
	const MapComponent = React.memo(
		({
			center,
			places,
		}: { center: { lat: number; lng: number }; places: any[] }) => {
			...
			const initializeMap = useCallback(async () => {
				if (
					mapRef.current &&
					isValidCoordinate(memoizedCenter.lat) &&
					isValidCoordinate(memoizedCenter.lng)
				) {
					...
					memoizedPlaces.forEach((place) => {
						if (
							isValidCoordinate(place.location.lat) &&
							isValidCoordinate(place.location.lng)
						) {
	...
	const MapSkeleton = () => <Skeleton className="w-full h-64" />;
	...
	const MapEmbed = memo(
		({ location, zoom = 15 }: { location: string; zoom?: number }) => {
	...
	const TextSearchResult = memo(({ result }: { result: any }) => {
		...
		const mapLocation = centerLocation
	...
	const renderToolInvocation = (
					...
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							...
							<span>
								Nearby{" "}
								{args.type
									? args.type.charAt(0).toUpperCase() + args.type.slice(1) + "s"
									: "Places"}
							</span>
							{args.keyword && (
								<Badge variant="secondary">{args.keyword}</Badge>
							)}
						</CardTitle>
					</CardHeader>
						...
						{result?.images && result.images.length > 0 && (
							<TabsContent value="images" className="p-0 m-0 bg-white">
								<div className="space-y-4 p-4">
									{result.images.map(
										(
											img: { format: "png" | "jpeg" | "svg"; data: string },
											imgIndex: number,
										) => (
		...
		if (toolInvocation.toolName === "nearby_search") {
			...
			const mapUrl = `https://www.google.com/maps/embed/v1/search?key=${env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&q=${encodeURIComponent(args.type)}&center=${result.results[0].geometry.location.lat},${result.results[0].geometry.location.lng}&zoom=14`;
				...
				<Card className="w-full my-4 overflow-hidden">
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<MapPin className="h-5 w-5 text-primary" />
							<span>
								Nearby {args.type.charAt(0).toUpperCase() + args.type.slice(1)}s
							</span>
						</CardTitle>
					</CardHeader>
					<CardContent className="p-0">
						...
						<AccordionItem value={`item-${index}`} className="border-none">
							...
							<AccordionContent className="pb-2">
								{args?.query && (
									<Badge
										variant="secondary"
										className="mb-2 text-xs sm:text-sm font-light rounded-full"
									>
								...
								{result && (
									<div className="flex flex-row gap-4 overflow-x-scroll">
										{result.results.map((item: any, itemIndex: number) => (
											<Card
												key={itemIndex}
												className="flex flex-col !size-40 shadow-none !p-0 !m-0"
											>
												...
												<div className="px-1 py-2 bg-muted rounded-b-xl">
													...
													>
														↪
														<span className="ml-1 truncate hover:underline">
															{item.url}
														</span>
													</a>
												</div>
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								...
								onClick={() =>
									window.open(
										"https://github.com/sponsors/zaidmukaddam",
										"_blank",
									)
								}
								className="flex items-center space-x-2"
	...
	return (
		<div className="flex flex-col font-sans items-center justify-center p-2 sm:p-4 bg-background text-foreground transition-all duration-500">
			...
			<div
				className={`w-full max-w-[90%] sm:max-w-2xl space-y-6 p-1 ${hasSubmitted ? "mt-16 sm:mt-20" : "mt-[26vh] sm:mt-[30vh]"}`}
			>
				{!hasSubmitted && (
					<div className="text-center">
						<h1 className="text-4xl sm:text-6xl mb-1 text-primary font-serif">
							MiniPerplx
						</h1>
					...
					{!hasSubmitted && (
						...
						>
							<form
								...
								className="flex items-center space-x-2 px-2 mb-4 sm:mb-6"
							>
							...
							<div
								className={`mx-auto w-full transition-all ${showExamples ? "visible" : "invisible"}`}
							>
